...
public class DockNode extends VBox implements EventHandler<MouseEvent> {
   ...
   */
	protected DockPos dockedPos = null;
	
	protected Boolean scaleable = false;
	
	protected Boolean takeOtherDocks = true;
	
	protected Boolean dockable = true;
	
	protected Boolean dockInsideOthers = true;
  ...
  protected StageStyle stageStyle = StageStyle.TRANSPARENT;
  /**
   * The stage that this dock node is currently using when floating.
   */
  protected Stage stage;
	
  protected DoubleProperty dockSize = new SimpleDoubleProperty(0);
   ...
   */
  protected static final PseudoClass FLOATING_PSEUDO_CLASS = PseudoClass.getPseudoClass("floating");
  /**
   * CSS pseudo class selector representing whether this node is currently docked.
   */
  protected static final PseudoClass DOCKED_PSEUDO_CLASS = PseudoClass.getPseudoClass("docked");
  /**
   * CSS pseudo class selector representing whether this node is currently maximized.
   */
  protected static final PseudoClass MAXIMIZED_PSEUDO_CLASS = PseudoClass.getPseudoClass("maximized");
   ...
   * @defaultValue false
   */
  protected BooleanProperty maximizedProperty = new SimpleBooleanProperty(false) {
  ...
  public DockNode(Node contents, String title, Node graphic) {
	  this.widthProperty().addListener((v,o,n)->{
		  if (dockedPos == DockPos.LEFT || dockedPos == DockPos.RIGHT){
			  //setDockSize((double)n);
		  }
	  });
	  this.heightProperty().addListener((v,o,n)->{
		  if (dockedPos == DockPos.BOTTOM || dockedPos == DockPos.TOP){
			  //setDockSize((double)n);
		  }
	  });
    DockSplit.setResizableWithParent(this, false);
  ...
  public DockNode(Node contents,Boolean isStatic) {
	  this.widthProperty().addListener((v,o,n)->{
		  if (dockedPos == DockPos.LEFT || dockedPos == DockPos.RIGHT){
			  setDockSize((double)n);
		  }
  ...
  protected ObjectProperty<Node> graphicProperty = new SimpleObjectProperty<Node>() {
    @Override
    public String getName() {
      return "graphic";
    }
  ...
  protected StringProperty titleProperty = new SimpleStringProperty("Dock") {
    @Override
    public String getName() {
      return "title";
    }
  ...
  protected BooleanProperty customTitleBarProperty = new SimpleBooleanProperty(true) {
    @Override
    public String getName() {
  ...
  protected BooleanProperty floatingProperty = new SimpleBooleanProperty(false) {
    @Override
    protected void invalidated() {
  ...
  protected BooleanProperty floatableProperty = new SimpleBooleanProperty(true) {
    @Override
    public String getName() {
      return "floatable";
    }
  ...
  protected BooleanProperty closableProperty = new SimpleBooleanProperty(true) {
    @Override
    public String getName() {
      return "closable";
    }
  ...
  protected BooleanProperty stageResizableProperty = new SimpleBooleanProperty(true) {
    @Override
    public String getName() {
      return "resizable";
    }
  ...
  protected BooleanProperty dockedProperty = new SimpleBooleanProperty(false) {
    @Override
    protected void invalidated() {
      if (get()) {
        if (dockTitleBar != null) {
  ...
  public final void setMaxiimzable(Boolean b){
	maximizable = b;  
  }
  ...
  protected final void dockImpl(DockPane dockPane) {
    if (isFloating()) {
      setFloating(false);
    }
  ...
  public void setDockProperty(Boolean b){
	  this.dockedProperty.set(b);
  }
  /**
   ...
   */
  protected Point2D sizeLast;
  /**
   * Whether we are currently resizing in a given direction.
   */
  protected boolean sizeWest = false, sizeEast = false, sizeNorth = false, sizeSouth = false;
  ...
  public void handle(MouseEvent event) {
    ...
    if (event.getEventType() == MouseEvent.MOUSE_PRESSED) {
  	...
  	public Boolean canTakeDocks(){
  		return takeOtherDocks;
  	}
  	public void setTakeDocks(Boolean b){
  		takeOtherDocks = b;
  	}
  	public Boolean  getScaleable(){
  		return scaleable;
  	}
  	public void setDockInsideOthers(Boolean b){
  		dockInsideOthers = b;
  	}
  	public Boolean getDockInsideOthers(){
  		return dockInsideOthers;
  	}
  	
	public void setScaleable(Boolean b){	
		scaleable = b;
		DockSplit.setResizableWithParent(this, b);
	}
	
	public void setDockPos(DockPos p){
		dockedPos = p;
	}
	...
	public void setDockSize(double i){
		this.dockSize.set(i);
	}
