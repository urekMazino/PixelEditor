...
public class DockTitleBar extends HBox implements EventHandler<MouseEvent> {
	...
	ArrayList<Node> activeButtons = new ArrayList<Node>();
  /**
   ...
   */
  private Label label,closeButton, stateButton;
  
  private boolean stateButtonHidden=false;
  /**
  ...
  public DockTitleBar(DockNode dockNode) {
    ...
    dockNode.floatingProperty().addListener((v,o,n)->{
    	setUpBar();
    });
    closeButton = new Label("c");
  ...
  public void hideStateButton(){
	  if (getChildren().contains(stateButton)){
		 getChildren().remove(stateButton);
		 stateButtonHidden = true;
	  }
  }
  void setUpBar(){
	  ...
	  children.addAll(label, fillPane);
	  if (!(!dockNode.isMaximizable() && dockNode.isFloating())){
		  if (!stateButtonHidden)
			  children.add(getStateButton());
	  } 
   ...
   */
  public final Label getStateButton() {
    return stateButton;
  }
